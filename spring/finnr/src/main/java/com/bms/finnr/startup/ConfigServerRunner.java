...
public class ConfigServerRunner implements ApplicationRunner, Ordered {
	...
	public Map<String, Object> loadConfigServerProperties(Configuration config, String tierName, String componentName,
		...
		for (ConfigProperty cps : cp) {
			String n = cps.getName().toUpperCase().replaceAll("_", "").replaceAll("-","");
			if (fieldMap.containsKey(n)) {
				...
				try {
					...
					// GlobalConfig: only use config server value if we haven't set this field
					// locally
	...
	private void loadTierGlobalProperties() {
		...
		String componentName = ApplicationConstants.COMPONENTNM_AWS_S3_BUCKETS;
		String name = "";	
		String auditComponent = appConfig.getComponentName();
		
		Map<String, Object> csmap = loadConfigServerProperties(globalConfig, tierName, componentName, name,
				auditComponent);
